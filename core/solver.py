...
class Solver(nn.Module):
    def __init__(self, args):
        ...
        if args.mode == 'train':
            ...
            self.ckptios = [
                CheckpointIO(ospj(args.checkpoint_dir, '{:06d}_nets.ckpt'.format(args.resume_iter)), data_parallel=True, **self.nets),
                CheckpointIO(ospj(args.checkpoint_dir, '{:06d}_nets_ema.ckpt'.format(args.resume_iter)), data_parallel=True, **self.nets_ema),
                CheckpointIO(ospj(args.checkpoint_dir, '{:06d}_optims.ckpt'.format(args.resume_iter)), **self.optims)]
